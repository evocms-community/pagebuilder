...
var initcontentblocks = function(opts) {
    return (function($) {
        var ContentBlock = {
            initializeContainers: function($containers) {
                $containers.each(function() {
                    ...
                    var $actions = $container.parent().children('.btn-group');

                    (function($container) {
                        $actions.on('click', '.export', function(e) {
                            e.preventDefault();

                            var name   = $container.attr('data-container'),
                                $items = $container.children('.block'),
                                values = [];

                            $items.each(function(i) {
                                values[i] = ContentBlock.fetch($(this));
                            });
                        ...
                        $actions.on('change', '[name="import-file"]', function(e) {
                            if ( e.target.files && e.target.files.length ) {
                                ...
                                reader.onload = (function(file) {
                                    return function(e) {
                                        ...
                                        var $last = $container.children('.block').last();

                                        if (!$last.length) {
                                            $last = $container.children('.add-block');
                                        }

                                        for (var i = json.length - 1; i >= 0; i--) {
                                            var $block = ContentBlock.append(json[i].config, $last, false);
            ...
            initialize: function(blocks) {
                $(blocks).each(function() {
                    ...
                    $wrap.on('click', '.restore-block > span', function(e) {
                        ...
                        var $removed = $(this).closest('.removed'),
                            data     = $removed.data('restore');

                        if (data) {
                            var $block = ContentBlock.append(data.config, $removed, false);
                            ContentBlock.setValues(opts.config[data.config].fields, $block.find('.fields-list').first(), data.values);
                            ContentBlock.initialize($block);
                        }
                    ...
                    $block.find('.field:not(.powered)').each(function() {
                        var $field = $(this).addClass('powered');

                        // add button for mass upload
                        $field.children('.sortable-list').each(function() {
                            var $list    = $(this),
                            ...
                            $list.on('click', '> .sortable-item > .controls > .insert', function(e) {
                                e.preventDefault();
                                ContentBlock.insertItem($(this).closest('.sortable-item'));
                            });

                            $list.on('click', '> .sortable-item > .controls > .remove', function(e) {
                                e.preventDefault();
                            ...
                            $list.prev('.group-title').on('click', '.fill-with', function(e) {
                                e.preventDefault();
                                var $list = $(this).closest('.group-title').next('.sortable-list'),
                        ...
                        $field.children('.open-browser').click(function(e) {
                            e.preventDefault();
                            ContentBlock.openBrowser($(this).next('input'), $(this).parent().hasClass('type-image') ? 'images' : 'files');
                        });

                        $field.children('.preview').click(function(e) {
                            $(this).next('.btn').click();
                        });
                    });

                    $block.children('.change-type').children('select').change(function(e) {
            ...
            fetch: function($block, old, skipContainers) {
                var config = false;
                ...
                for (var field in conf) {
                    ...
                    switch (conf[field].type) {
                        ...
                        case 'container': {
                            ...
                            var $container = $field.children('.content-blocks');

                            if (skipContainers) {
                                values[field] = $container.attr('data-hash');
                                break;
                            }

                            $container.children('.block').each(function(i) {
                                values[field][i] = ContentBlock.fetch($(this), {values: values[field] && values[field][i] ? values[field][i] : {}}, skipContainers);
                            });
            ...
            setValues: function(conf, $list, data) {
                for (var field in conf) {
                    var $field = $list.children('.field[data-field="' + field + '"]'),
                    ...
                    switch (conf[field].type) {
                        case 'group': {
                            if (typeof values[field] === 'object' && values[field].length) {
                                ...
                                for (i = 0; i < values[field].length; i++) {
                        ...
                        case 'container': {
                            if (typeof values[field] === 'object' && values[field].length) {
                                var $after = $field.find('.add-block'),
                                    $block, blockData;

                                for (i = 0; i < values[field].length; i++) {
                                    blockData = values[field][i];
                                    $block = ContentBlock.append(blockData.config, $after, false);
                                    ContentBlock.setValues(opts.config[blockData.config].fields, $block.children('.block-inner').children('.fields-list'), blockData);
                                    ContentBlock.initialize($block);
                                    $after = $block;
                                }
                            }
            ...
            append: function(config, $after, initialize) {
            ...
            initializeFieldsList: function(conf, $list) {
                ...
                for (var field in conf) {
                    ...
                    switch (fieldConfig.type) {
                        ...
                        case 'container': {
                            ContentBlock.initializeContainers($field.children('.content-blocks'));
                            break;
                        }
                    }
                }
            ...
            randomString: function() {
                var randoms = new Uint32Array(4), str = '';
                window.crypto.getRandomValues(randoms);

                for (var i = 0; i < randoms.length; i++) {
                    str += randoms[i].toString(16).slice(-4);
                }

                return str;
            },

            prepareSaving: function($form, $containers) {
                $containers.each(function() {
                    if ($(this).parents('.sortable-item.hidden').length) {
                        return;
                    }

                    var length = 0,
                        $container = $(this),
                        container = $container.attr('data-container'),
                        $rootContainer = $container.parents('.content-blocks').last(),
                        $blocks = $container.children('.block'),
                        isChild = $rootContainer.length;

                    if ($blocks.length) {
                        var hash = $container.attr('data-hash'),
                            rootContainer = $rootContainer.attr('data-container'),
                            $parentContainer = $container.parents('.content-blocks').first();

                        if (isChild) {
                            var $parentBlock = $container.closest('.block');
                            var data = {
                                subcontainer: container,
                                parenthash: $parentContainer.get(0) == $rootContainer.get(0) ? 'root' : $parentContainer.attr('data-hash'),
                                parentindex: $parentBlock.parent().children('.block').index($parentBlock)
                            };

                            for (var field in data) {
                                $('<input type="hidden"/>').attr('name', 'contentblocks[' + $rootContainer.attr('data-container') + '][' + hash + '][' + field + ']').val(data[field]).appendTo($form);
                            }
                        }
                        ...
                        $blocks.each(function() {
                            var $block = $(this),
                                ...
                                data = {
                                    ...
                                    config:  $block.attr('data-config'),
                                    values:  JSON.stringify(ContentBlock.fetch($inner, null, true))
                                };
                            ...
                            for (var field in data) {
                                $('<input type="hidden"/>').attr('name', 'contentblocks[' + (isChild ? rootContainer : container) + '][' + (isChild ? hash : 'root') + '][values][' + length + '][' + field + ']').val(data[field]).appendTo($form);
                            }

                            length++;
                        });

                        $container.find('[name]').attr('disabled', true);
                    }
        ...
        opts.containers.eq(0).closest('form').submit(function() {
            ...
            ContentBlock.prepareSaving($form, $('.content-blocks').filter(function(i, container) {
                return !$(container).parents('.content-blocks-configs').length;
            }));
