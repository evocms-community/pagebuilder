<div class="block" data-config="<?= $block['config'] ?>">
        ...
        <div class="fields-list">
            ...
            <?php else: ?>
                <?php foreach ($configs[$block['config']]['fields'] as $name => $field): ?>
                    <?= $this->renderField($field, $name, isset($block['values'][$name]) ? $block['values'][$name] : null, $block); ?>
                <?php endforeach; ?>
            <?php endif; ?>
        </div>
    ...
    <div class="restore-block-wrapper">
        <span class="restore-block">Блок удален. <span>Восстановить</span></span>
    </div>
